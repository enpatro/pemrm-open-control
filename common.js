...
function judge(value,target){ if(!isNum(value)) return {cell:'blankcell', ok:true, symbol:'', band:'blank'}; let t=parseTarget(target); if(!t) return {cell:'blankcell', ok:true, symbol:'', band:'info'}; let v=Number(value), ok=true, ratio=null;
 if(t.op==='<') {ok=v<t.v; ratio=t.v? v/t.v: null;} else if(t.op==='<=') {ok=v<=t.v; ratio=t.v? v/t.v: null;} else if(t.op==='>') {ok=v>t.v; ratio=t.v? v/t.v: null;} else if(t.op==='>=') {ok=v>=t.v; ratio=t.v? v/t.v: null;} else {ratio=t.v===0? (v===0?1:999) : v/t.v; ok=(t.v===0? v===0 : (ratio>=0.8 && ratio<=1.2));}
 let symbol='○'; if(ratio!==null){ if(ratio>=0.8 && ratio<=1.2) symbol='○'; else if(ratio<0.8) symbol='▲'; else symbol='■'; } if(t.v===0 && v!==0) symbol='■'; return {cell:ok?'okcell':'redcell', ok, symbol, band:ok?'ok':'ng'}; }
