...
import { getFirestore, doc, setDoc, getDoc, collection, getDocs, serverTimestamp, query, where, enableNetwork } from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';
...
async function getMonthFB(col,month){ requireFirebase(); const out={}; const qs=await getDocs(query(collection(db,col), where('month','==',month))); qs.forEach(x=>out[x.id]=x.data()); return out; }
async function getAllMonthsFB(col){ requireFirebase(); const out={}; const qs=await getDocs(collection(db,col)); qs.forEach(x=>out[x.id]=x.data()); return out; }
